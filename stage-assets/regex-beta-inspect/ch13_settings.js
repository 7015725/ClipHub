(function (global) {
    ...
    var paginationModeAjaxView = null;
    var paginationModeNumberView = null;
    var paginationPageSizeInput = null;
    var paginationPrefetchView = null;
    var paginationStatusView = null;
    var paginationSectionView = null;
    var draftPaginationMode = "ajax";
    var draftPaginationPrefetchEnabled = true;
    ...
    var settingsClosing = false;
    var settingsRemovalPending = false;
    var settingsRemovalGeneration = 0;
    ...
    var uiState = {
        ...
        clearHistoryCount: 0,
        paginationSaveCount: 0,
        paginationValidationFailureCount: 0,
        paginationChangeEventCount: 0,
        paginationSectionPresent: false,
        paginationPageSizeInputBound: false,
        paginationLastError: null,
    ...
    function normalizePaginationPageSize(value) {
        ...
        if (!isFinite(number) || Math.floor(number) !== number ||
                number < 5 || number > 1000) {
            throw new Error("分页数量必须是 5-1000 的整数");
        }
        return number;
    }
    ...
    function normalize(key, value) {
        ...
        if (key === "closeAfterCopy" || key === "sourceEnabled" ||
                ...
                key === "paginationPrefetchEnabled" ||
                key === "translation.auto_direction") {
            return value === true;
        }
        if (key === "paginationMode") {
            value = String(value || "");
            if (value !== "ajax" && value !== "number") {
                throw new Error("分页模式必须是 ajax 或 number");
            }
            return value;
        }
        if (key === "paginationPageSize") {
            return normalizePaginationPageSize(value);
        }
    ...
    function setValue(key, value, options) {
        ...
        if (key === "paginationMode" || key === "paginationPageSize" ||
                key === "paginationPrefetchEnabled") {
            emitPaginationSettingsChanged("set");
        }
    ...
    function setMany(patch, options) {
        ...
        var needsPaginationEvent = false;
        options = options || {};
        requireReady();
        patch = patch || {};
        for (key in patch) {
        ...
        for (key in normalized) {
            if (normalized.hasOwnProperty(key)) {
                ...
                if (key === "paginationMode" ||
                        ...
                        key === "paginationPrefetchEnabled") {
                    needsPaginationEvent = true;
                }
            }
        }
        if (needsClipboard) { applyClipboard(); }
        if (needsCleanup && options.cleanup !== false) { cleanup(); }
        if (needsPaginationEvent) {
            emitPaginationSettingsChanged("setMany");
        }
        return getAll();
    }
    ...
    function getPaginationSettings() {
        return {
            mode: String(values.paginationMode || "ajax"),
            pageSize: Number(values.paginationPageSize || 100),
            prefetchEnabled: values.paginationPrefetchEnabled === true
        };
    }

    function emitPaginationSettingsChanged(origin) {
        var payload = getPaginationSettings();
        payload.origin = String(origin || "unknown");
        payload.at = ClipHub.Base.now();
        uiState.paginationChangeEventCount += 1;
        try {
            if (ClipHub.EventBus &&
                    typeof ClipHub.EventBus.emit === "function") {
                ClipHub.EventBus.emit("pagination_settings_changed", payload);
            }
        } catch (ignored) {}
        return payload;
    }

    function updatePaginationViews(colors) {
        var ajaxSelected = draftPaginationMode === "ajax";
        if (paginationModeAjaxView !== null) {
            paginationModeAjaxView.setText(
                ajaxSelected ? "AJAX 追加 ✓" : "AJAX 追加"
            );
            paginationModeAjaxView.setBackground(roundedBackground(
                ajaxSelected ? colors.accentStrong : colors.accentSoft,
                colors.accentBorder, 11));
            ClipHub.Theme.applyTextColor(paginationModeAjaxView,
                ajaxSelected ? "#FFFFFFFF" : colors.accentStrong);
        }
        if (paginationModeNumberView !== null) {
            paginationModeNumberView.setText(
                ajaxSelected ? "数字分页" : "数字分页 ✓"
            );
            paginationModeNumberView.setBackground(roundedBackground(
                ajaxSelected ? colors.accentSoft : colors.accentStrong,
                colors.accentBorder, 11));
            ClipHub.Theme.applyTextColor(paginationModeNumberView,
                ajaxSelected ? colors.accentStrong : "#FFFFFFFF");
        }
        if (paginationPrefetchView !== null) {
            paginationPrefetchView.setText(
                draftPaginationPrefetchEnabled ?
                    "下一页预加载：开启" : "下一页预加载：关闭"
            );
        }
    }
    ...
    function savePaginationSettings() {
        ...
        try {
            saved = setMany({
                paginationMode: draftPaginationMode,
                paginationPageSize: paginationPageSizeInput === null ?
                    values.paginationPageSize :
                    String(paginationPageSizeInput.getText()),
                paginationPrefetchEnabled:
                    draftPaginationPrefetchEnabled
            }, { cleanup: false });
            draftPaginationMode = String(saved.paginationMode || "ajax");
            draftPaginationPrefetchEnabled =
                saved.paginationPrefetchEnabled === true;
            uiState.paginationSaveCount += 1;
            uiState.paginationLastError = null;
            if (paginationStatusView !== null) {
                paginationStatusView.setText(
                    "已保存 · 每次 " +
                    String(Number(saved.paginationPageSize || 100)) +
                    " 条 · " +
                    (draftPaginationMode === "ajax" ?
                        "AJAX 追加" : "数字分页")
                );
            }
            updatePaginationViews(palette());
            return getPaginationSettings();
        } catch (error) {
            uiState.paginationValidationFailureCount += 1;
            uiState.paginationLastError = String(error);
            uiState.lastError = String(error);
            if (paginationStatusView !== null) {
                paginationStatusView.setText(
                    "保存失败：" + String(error)
    ...
    function setPaginationDraft(mode, pageSize, prefetchEnabled) {
        mode = String(mode || "");
        if (mode !== "ajax" && mode !== "number") {
            throw new Error("分页模式必须是 ajax 或 number");
        }
        draftPaginationMode = mode;
        draftPaginationPrefetchEnabled = prefetchEnabled === true;
        if (paginationPageSizeInput !== null) {
            paginationPageSizeInput.setText(String(pageSize));
            paginationPageSizeInput.setSelection(
                paginationPageSizeInput.getText().length()
            );
        }
        updatePaginationViews(palette());
        return {
            mode: draftPaginationMode,
            pageSizeText: paginationPageSizeInput === null ?
                String(pageSize) :
                String(paginationPageSizeInput.getText()),
            prefetchEnabled: draftPaginationPrefetchEnabled
        };
    }
    ...
    function makePaginationSection(colors) {
        ...
        makeSectionTitle(section, "首页分页",
            "选择加载方式、单次数量和下一页预加载", colors);
        draftPaginationMode = String(values.paginationMode || "ajax");
        draftPaginationPrefetchEnabled =
            values.paginationPrefetchEnabled === true;

        modeRow.setOrientation(LinearLayout.HORIZONTAL);
        paginationModeAjaxView = makeButton(
            "AJAX 追加", colors, false, false);
        paginationModeNumberView = makeButton(
            "数字分页", colors, false, false);
        paginationModeAjaxView.setOnClickListener(new JavaAdapter(
            View.OnClickListener, { onClick: function () {
                draftPaginationMode = "ajax";
                updatePaginationViews(colors);
            }}));
        paginationModeNumberView.setOnClickListener(new JavaAdapter(
            View.OnClickListener, { onClick: function () {
                draftPaginationMode = "number";
                updatePaginationViews(colors);
            }}));
        params = new LinearLayout.LayoutParams(0, dp(40), 1);
        params.rightMargin = dp(6);
        modeRow.addView(paginationModeAjaxView, params);
        modeRow.addView(paginationModeNumberView,
            new LinearLayout.LayoutParams(0, dp(40), 1));
        section.addView(modeRow, new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT, dp(40)));

        paginationPageSizeInput = makeInput(
            "5-1000", String(values.paginationPageSize || 100),
            colors, false, "paginationPageSize");
        paginationPageSizeInput.setInputType(
            InputType.TYPE_CLASS_NUMBER
        );
        paginationPageSizeInput.setSelectAllOnFocus(true);
        addField(section, "单次加载数量（5-1000）",
            paginationPageSizeInput, colors);
        uiState.paginationPageSizeInputBound = true;
        ...
        params = new LinearLayout.LayoutParams(
            ...
            LinearLayout.LayoutParams.WRAP_CONTENT);
        params.bottomMargin = dp(7);
        section.addView(help, params);

        actionRow.setOrientation(LinearLayout.HORIZONTAL);
        paginationPrefetchView = makeButton(
            "", colors, false, false);
        paginationPrefetchView.setOnClickListener(new JavaAdapter(
            View.OnClickListener, { onClick: function () {
                draftPaginationPrefetchEnabled =
                    !draftPaginationPrefetchEnabled;
                updatePaginationViews(colors);
        ...
        saveButton = makeButton(
            "保存分页设置", colors, true, false);
        saveButton.setOnClickListener(new JavaAdapter(
            View.OnClickListener, { onClick: savePaginationSettings }));
        params = new LinearLayout.LayoutParams(0, dp(40), 1);
        params.rightMargin = dp(6);
        actionRow.addView(paginationPrefetchView, params);
        actionRow.addView(saveButton,
            new LinearLayout.LayoutParams(0, dp(40), 1));
        section.addView(actionRow, new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT, dp(40)));

        paginationStatusView = makeText(
            "设置保存到 ClipHub SQLite；首页接入将在下一阶段启用。",
            9, colors.textSecondary, false);
        params = new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT,
            LinearLayout.LayoutParams.WRAP_CONTENT);
        params.topMargin = dp(7);
        section.addView(paginationStatusView, params);

        paginationSectionView = section;
        uiState.paginationSectionPresent = true;
        updatePaginationViews(colors);
        return section;
    }
    ...
    function buildPage() {
        ...
        paginationSectionView = makePaginationSection(colors);
        addSection(content, paginationSectionView);
    ...
    function closePage(reason) {
        ...
        if (!uiState.attached && panelRoot === null &&
                !settingsRemovalPending) { return true; }
        if (settingsRemovalPending) { return true; }
        return runOnMainSync(function () {
            ...
            settingsRemovalPending = capturedRoot !== null;
            settingsRemovalGeneration += 1;
            generation = settingsRemovalGeneration;
            ...
            paginationModeAjaxView = null;
            paginationModeNumberView = null;
            paginationPageSizeInput = null;
            paginationPrefetchView = null;
            paginationStatusView = null;
            paginationSectionView = null;
            uiState.paginationSectionPresent = false;
            uiState.paginationPageSizeInputBound = false;
            tagRowViews = {};
            ...
            removal = ClipHub.Window.requestViewRemoval({
                ...
                onDetached: function (result) {
                    var reopen;
                    if (generation !== settingsRemovalGeneration) { return; }
                    settingsClosing = false;
                    settingsRemovalPending = false;
                    if (!result || result.ok !== true) {
                        uiState.lastError = String(result && result.error ?
                            result.error : "Settings removal failed");
                        pendingSettingsOpen = false;
                        return;
                    }
                    reopen = pendingSettingsOpen;
                    pendingSettingsOpen = false;
                    if (reopen && ready) { openPage(); }
                }
            });
            if (!removal || removal.ok !== true) {
                ...
                settingsRemovalPending = false;
                pendingSettingsOpen = false;
                uiState.lastError = String(removal && removal.error ?
                    removal.error : "Settings removal queue failed");
            }
    ...
    function getState() {
        ...
        return {
            ...
            closing: settingsClosing === true,
            removalPending: settingsRemovalPending === true,
            ...
            paginationPageSizeInputBound:
                uiState.paginationPageSizeInputBound === true,
            paginationMode: String(values.paginationMode || "ajax"),
            paginationPageSize:
                Number(values.paginationPageSize || 100),
            paginationPrefetchEnabled:
                values.paginationPrefetchEnabled === true,
            paginationDraftMode: String(draftPaginationMode || "ajax"),
            paginationDraftPrefetchEnabled:
                draftPaginationPrefetchEnabled === true,
            paginationSaveCount:
                Number(uiState.paginationSaveCount),
            paginationValidationFailureCount:
                Number(uiState.paginationValidationFailureCount),
            paginationChangeEventCount:
                Number(uiState.paginationChangeEventCount),
            paginationLastError: uiState.paginationLastError,
    ...
    ClipHub.Settings = {
        ...
        performSavePaginationClick: function () {
            return runOnMainSync(savePaginationSettings, 3000);
        },
        performSetPaginationDraft: function (mode, pageSize,
                prefetchEnabled) {
            return runOnMainSync(function () {
                return setPaginationDraft(
                    mode, pageSize, prefetchEnabled);
            }, 3000);
        },
        ...
        reset: function (options) {
            ...
            values = defaults;
            applyClipboard();
            emitPaginationSettingsChanged("reset");
