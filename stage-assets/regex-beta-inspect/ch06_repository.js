(function (global) {
    ...
    function normalizePageSize(value) {
        ...
        if (!isFinite(number) || Math.floor(number) !== number ||
                number < MIN_PAGE_SIZE || number > MAX_PAGE_SIZE) {
    ...
    function copyArray(values) {
        return values instanceof Array ? values.slice(0) : [];
    }
    ...
    function insertItem(item) {
        ...
        requireReady();
        item = item || {};
        content = String(item.content === null ||
    ...
    function buildItemWhere(options) {
        ...
        if (!options.includeDeleted) {
            where.push("clipboard_items.deleted_at IS NULL");
        }
        keyword = String(options.keyword === null ||
        ...
        if (tagIds.length > 0) {
            where.push("EXISTS (SELECT 1 FROM clipboard_item_tags " +
                "filter_tags WHERE filter_tags.item_id = " +
                "clipboard_items.id AND filter_tags.tag_id IN (" +
        ...
        if (options.sensitiveOnly) {
            where.push("clipboard_items.is_sensitive = 1");
        }
        if (options.excludeSensitive) {
            where.push("clipboard_items.is_sensitive = 0");
        }
        if (options.pinnedOnly) {
            where.push("clipboard_items.is_pinned = 1");
        }
        return {
            sql: where.length > 0 ? where.join(" AND ") : "1 = 1",
            args: args,
    ...
    function normalizedSourceSql() {
        return "LOWER(COALESCE(" +
            "NULLIF(clipboard_items.source_label, ''), " +
            "NULLIF(clipboard_items.source_package, ''), ''))";
    }

    function buildItemOrder(sortMode) {
        sortMode = normalizeSortMode(sortMode);
        if (sortMode === "source") {
            return normalizedSourceSql() + " ASC, " +
                "clipboard_items.is_pinned DESC, " +
                "clipboard_items.manual_order ASC, " +
                "clipboard_items.last_copied_at DESC, " +
                "clipboard_items.id DESC";
        }
        return "clipboard_items.is_pinned DESC, " +
            "clipboard_items.manual_order ASC, " +
            "clipboard_items.last_copied_at DESC, " +
            "clipboard_items.id DESC";
    }

    function buildPageCursor(row, sortMode) {
        if (row === null || row === undefined) { return null; }
        sortMode = normalizeSortMode(sortMode);
        return {
            sortMode: sortMode,
            sourceKey: sortMode === "source" ?
                String(row.source_label || row.source_package || "")
                    .toLowerCase() : "",
            isPinned: Number(row.is_pinned || 0),
            manualOrder: Number(row.manual_order || 0),
            lastCopiedAt: Number(row.last_copied_at || 0),
            id: Number(row.id || 0)
        };
    }

    function buildStableTailCursorWhere(cursor, args) {
        args.push(Number(cursor.isPinned || 0));
        args.push(Number(cursor.isPinned || 0));
        args.push(Number(cursor.manualOrder || 0));
        args.push(Number(cursor.manualOrder || 0));
        args.push(Number(cursor.lastCopiedAt || 0));
        args.push(Number(cursor.lastCopiedAt || 0));
        args.push(Number(cursor.id || 0));
        return "(clipboard_items.is_pinned < ? OR (" +
            "clipboard_items.is_pinned = ? AND (" +
            "clipboard_items.manual_order > ? OR (" +
            "clipboard_items.manual_order = ? AND (" +
            "clipboard_items.last_copied_at < ? OR (" +
            "clipboard_items.last_copied_at = ? AND " +
            "clipboard_items.id < ?))))))";
    }

    function buildAppendCursorWhere(sortMode, cursor, args) {
        var sourceKey;
        var tailArgs = [];
        var tailSql;
        args = args || [];
        ...
        if (cursor === null || cursor === undefined ||
                Number(cursor.id || 0) < 1) {
            return "";
        }
        tailSql = buildStableTailCursorWhere(cursor, tailArgs);
        if (sortMode !== "source") {
            Array.prototype.push.apply(args, tailArgs);
            return tailSql;
        }
        sourceKey = String(cursor.sourceKey || "").toLowerCase();
        args.push(sourceKey);
        args.push(sourceKey);
        Array.prototype.push.apply(args, tailArgs);
        return "(" + normalizedSourceSql() + " > ? OR (" +
            normalizedSourceSql() + " = ? AND " + tailSql + "))";
    }

    function itemSelectColumns(previewOnly) {
        if (previewOnly !== true) {
    ...
    function listItems(options) {
        ...
        requireReady();
        options = options || {};
        built = buildItemWhere(options);
        limit = positiveLimit(options.limit, 50, 1000);
        offset = intValue(options.offset, 0);
        if (offset < 0) { offset = 0; }
        sql = "SELECT " + itemSelectColumns(options.previewOnly === true) + " FROM clipboard_items WHERE " +
            built.sql + " ORDER BY " +
            buildItemOrder(options.sortMode) + " LIMIT ? OFFSET ?";
    ...
    function countItemsByOptions(options) {
        var built;
        requireReady();
        built = buildItemWhere(options || {});
    ...
    function listItemPage(options) {
        ...
        requireReady();
        options = options || {};
        mode = String(options.mode || "number") === "append" ?
            "append" : "number";
        pageSize = normalizePageSize(options.pageSize);
        requestedPage = normalizePageNumber(options.page);
        page = requestedPage;
        sortMode = normalizeSortMode(options.sortMode);
        built = buildItemWhere(options);
        if (mode === "number" || options.includeTotal === true) {
            totalCount = countItemsByOptions(options);
            totalPages = Math.max(1, Math.ceil(totalCount / pageSize));
            if (page > totalPages) { page = totalPages; }
        }
        whereParts = [built.sql];
        args = copyArray(built.args);
        if (mode === "append" && options.cursor) {
            cursorSql = buildAppendCursorWhere(
                sortMode, options.cursor, args);
            if (cursorSql) { whereParts.push(cursorSql); }
        }
        offset = mode === "number" ? (page - 1) * pageSize : 0;
        sql = "SELECT " + itemSelectColumns(options.previewOnly === true) + " FROM clipboard_items WHERE " +
            whereParts.join(" AND ") + " ORDER BY " +
            buildItemOrder(sortMode) + " LIMIT ? OFFSET ?";
        args.push(pageSize + 1);
        args.push(offset);
        startedAt = Number(System.nanoTime());
        queried = ClipHub.Database.queryAll(sql, args);
        endedAt = Number(System.nanoTime());
        hasMore = queried.length > pageSize;
        rows = hasMore ? queried.slice(0, pageSize) : queried;
        return {
            rows: rows,
            pageSize: pageSize,
            page: page,
            ...
            hasMore: hasMore,
            startCursor: rows.length > 0 ?
                buildPageCursor(rows[0], sortMode) : null,
            endCursor: rows.length > 0 ?
                buildPageCursor(rows[rows.length - 1], sortMode) : null,
            sortMode: sortMode,
            mode: mode,
            queryMs: Math.round(
                ((endedAt - startedAt) / 1000000) * 1000) / 1000,
            tagQueryMs: 0
        };
    }
    ...
    function listSourceOptions() {
        ...
        return ClipHub.Database.queryAll(
            ...
            "ORDER BY COALESCE(MAX(source_label), source_package) " +
            "COLLATE NOCASE ASC",
            []
        );
    }
    ...
    function listOrderRows(pinned) {
        requireReady();
        return ClipHub.Database.queryAll(
            "SELECT id, is_pinned, manual_order, last_copied_at " +
            "FROM clipboard_items WHERE deleted_at IS NULL " +
            "AND is_pinned = ? ORDER BY manual_order ASC, " +
    ...
    function reorderItem(itemId, targetId, placeAfter) {
        ...
        if (Number(source.id) === Number(target.id)) {
            return {
                ok: true, changed: false, reason: "same_item",
        ...
        if (Number(source.is_pinned) !== Number(target.is_pinned)) {
            return { ok: false, changed: false,
                reason: "cross_pinned_group" };
        }
        pinned = Number(source.is_pinned) === 1;
        rows = listOrderRows(pinned);
        for (index = 0; index < rows.length; index += 1) {
            if (Number(rows[index].id) === Number(source.id)) {
                sourceIndex = index;
            }
        ...
        if (targetIndex < 0) {
            return { ok: false, changed: false,
                reason: "target_missing_after_remove" };
        }
        ...
        ClipHub.Database.transaction(function () {
            for (index = 0; index < rows.length; index += 1) {
                changed += ClipHub.Database.executeUpdateDelete(
                    "UPDATE clipboard_items SET manual_order = ? " +
                    "WHERE id = ?",
    ...
    function trimHistory(limit) {
        ...
        return ClipHub.Database.executeUpdateDelete(
            "DELETE FROM clipboard_items WHERE is_pinned = 0 " +
            "AND id IN (SELECT id FROM clipboard_items " +
            "WHERE is_pinned = 0 ORDER BY last_copied_at DESC, " +
    ...
    function cleanupHistory(options) {
        ...
        result.totalDeleted =
            result.expiredDeleted + result.overflowDeleted;
    ...
    function getTag(id) {
        ...
        return ClipHub.Database.queryOne(
            ...
            "WHERE cit.tag_id = t.id AND ci.deleted_at IS NULL) " +
            "AS item_count FROM tags t WHERE t.id = ? LIMIT 1",
            [intValue(id, -1)]
        );
    }
    ...
    function updateTag(id, patch) {
        ...
        if (patch.hasOwnProperty("color_value")) {
            ...
            args.push(patch.color_value === null ||
                patch.color_value === undefined ?
                null : intValue(patch.color_value, 0));
        }
        if (patch.hasOwnProperty("colorValue")) {
            columns.push("color_value = ?");
            args.push(patch.colorValue === null ||
                patch.colorValue === undefined ?
    ...
    function listItemTagMap(itemIds) {
        ...
        rows = ClipHub.Database.queryAll(
            ...
            "t.color_value, t.manual_order " +
            "FROM clipboard_item_tags cit JOIN tags t " +
            "ON t.id = cit.tag_id WHERE cit.item_id IN (" +
            placeholders(ids.length) + ") " +
            "ORDER BY cit.item_id ASC, t.manual_order ASC, " +
    ...
    function listItemTagMapChunked(itemIds) {
        var ids = intList(itemIds);
        var result = {};
        var start;
        var chunk;
        requireReady();
        for (start = 0; start < ids.length;
                start += TAG_ID_CHUNK_SIZE) {
            chunk = ids.slice(
                start,
                Math.min(ids.length, start + TAG_ID_CHUNK_SIZE)
            );
            mergeTagMaps(result, listItemTagMap(chunk));
        }
        return result;
    }
    ...
    function listItemsByIds(itemIds, previewOnly) {
        ...
        for (start = 0; start < ids.length;
            ...
            rows = ClipHub.Database.queryAll(
                "SELECT " + itemSelectColumns(previewOnly === true) + " FROM clipboard_items " +
                "WHERE clipboard_items.deleted_at IS NULL AND " +
                "clipboard_items.id IN (" +
                placeholders(chunk.length) + ")",
                copyArray(chunk)
            );
            for (index = 0; index < rows.length; index += 1) {
                byId[String(rows[index].id)] = rows[index];
            }
        }
        for (index = 0; index < ids.length; index += 1) {
            if (byId[String(ids[index])]) {
                output.push(byId[String(ids[index])]);
            }
        }
        return output;
    }
    ...
    function replaceItemTagsInternal(itemId, tagIds) {
        ...
        for (index = 0; index < ids.length; index += 1) {
            if (attachTag(itemId, ids[index]) >= 0) {
                attached += 1;
            }
        }
        return attached;
    ...
    function saveItemWithTags(options) {
        ...
        ClipHub.Database.transaction(function () {
            ...
            tagCount = replaceItemTagsInternal(
                itemId,
                options.tagIds || []
            );
        });
    ...
    ClipHub.Repository = {
        ...
        init: function () {
            ready = !!(ClipHub.Database &&
                ClipHub.Database.isOpen());
            if (!ready) {
        ...
        listItems: listItems,
        buildItemWhere: buildItemWhere,
        buildItemOrder: buildItemOrder,
        buildAppendCursorWhere: buildAppendCursorWhere,
        buildPageCursor: buildPageCursor,
        listItemPage: listItemPage,
        countItemsByOptions: countItemsByOptions,
        listItemsByIds: listItemsByIds,
        listItemTagMapChunked: listItemTagMapChunked,
        TAG_ID_CHUNK_SIZE: TAG_ID_CHUNK_SIZE,
