...
1. 在首页列表上方点“多选”，再点词条或右侧选择按钮勾选。
2. 点“全选已加载”可选择已加载记录；每批最多 100 条，超过时选择已加载列表前 100 条。新到达的记录由用户另行勾选。
3. 点“删除”，核对数量后点“确认删除”；删除后可在底部提示条中撤销整批。
4. 返回键先退出删除确认，再退出多选。修改筛选、数字翻页、进入其他页面或关闭窗口会清空选择。

多选模式内暂停正文输入、长按分词和单条操作，退出后恢复。批量操作采用事务软删除，写入失败时整批回滚。正常模式长按词条继续进入分词。
