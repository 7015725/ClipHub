...
var visible = false;
var limit = 20;
var lastShowOptions = { widthDp: 340, heightDp: 500 };
var items = [];
var itemViews = [];
var cardContainers = [];
var deleteViews = [];
...
function makeActionButton(icon, label, palette, primary, danger) {
...
(danger ? palette.dangerSoft : "#00FFFFFF"),
null, 12));
iconView = makeText(icon, 18,
danger ? palette.danger :
(primary ? palette.accentStrong : palette.icon), false);
iconView.setGravity(Gravity.CENTER);
root.addView(iconView, new LinearLayout.LayoutParams(
LinearLayout.LayoutParams.MATCH_PARENT, dp(25)));
labelView = makeText(label, 10,
danger ? palette.danger : palette.textSecondary,
primary || danger);
...
var content = String(row && row.content !== undefined ? row.content : "");
var declared = String(row && row.content_type ? row.content_type : "text");
...
declared === "phone") {
return { key: "phone", label: "电话号码", icon: "☎",
...
if (selectAfter !== false) { selectRow(row); }
try {
...
} else if (visible && ClipHub.Window && ClipHub.Window.isAttached()) {
renderRows(items);
}
...
if (Number(selectedItemId) === id) { selectRow(null); }
state.deleteCount += 1;
state.lastDeletedId = id;
state.deleteThreadId = Number(thread.getId());
state.deleteThreadName = String(thread.getName());
delivered = emit("clipboard_deleted", { id: id,
deletedAt: deletedAt, threadId: state.deleteThreadId,
...
var width;
var height;
try { detailWindowManager.getDefaultDisplay().getRealMetrics(display); }
catch (ignored) {
...
return detailRestoreList && visible && reason !== "list_hide" &&
reason !== "shutdown" && reason !== "copy_close" &&
reason !== "replace";
}
...
heightDp: Number(lastShowOptions.heightDp || 500),
...
var title = makeText("内容详情", 17, palette.textPrimary, true);
var meta = makeText(sourceText(row) + "  ·  " +
formatTime(row.last_copied_at), 10, palette.textSecondary, false);
...
palette.stroke, 24));
if (Build.VERSION.SDK_INT >= 21) { root.setElevation(dp(22)); }
handle.setBackground(roundedBackground(palette.strokeStrong, null, 3));
...
detailCloseView = makeIcon("×", palette.textSecondary, 22,
"关闭内容详情");
detailCloseView.setBackground(circleBackground(palette.surfaceMuted,
palette.stroke));
...
try {
attached = detailRoot !== null && detailRoot.isAttachedToWindow();
} catch (ignored) {}
return {
...
try {
cardContainers[index].setAlpha(index === targetIndex ? 0.70 : 1);
} catch (ignored) {}
}
}
...
if (!reorderDrag.active || index !== reorderDrag.sourceIndex) {
return false;
}
...
reorderDrag.syntheticTargetIndex = null;
if (action === Number(MotionEvent.ACTION_UP) &&
reorderDrag.moved && commitFrom !== commitTo) {
reorderDrag.moved = false;
...
var icon = makeText(type.icon, type.key === "code" ? 9 : 16,
type.color, true);
icon.setGravity(Gravity.CENTER);
icon.setBackground(circleBackground(type.soft, null));
...
function buildCard(row, rowTags, index, allowReorder, palette) {
...
var check = makeText("✓", 11, "#FFFFFFFF", true);
var icon = makeSourceIcon(row, palette);
var center = new LinearLayout(androidContext);
var preview = makeText(Number(row.is_sensitive || 0) === 1 ?
"敏感内容" : String(row.content), 13, palette.textPrimary, false);
...
var time = makeText(formatTime(row.last_copied_at), 10,
palette.textTertiary, false);
var star = makeIcon(Number(row.is_pinned || 0) === 1 ? "★" : "☆",
Number(row.is_pinned || 0) === 1 ? palette.accentStrong :
palette.textTertiary, 20, "置顶");
var more = makeIcon("⋮", palette.icon, 19, "更多与拖动排序");
var type = typeInfo(row, palette);
...
selected ? palette.accent : palette.stroke, 15));
if (Build.VERSION.SDK_INT >= 21) { wrapper.setElevation(dp(1)); }
card.setOrientation(LinearLayout.HORIZONTAL);
card.setGravity(Gravity.CENTER_VERTICAL);
card.setPadding(dp(9), dp(9), dp(6), dp(9));
card.setClickable(true);
card.setFocusable(true);
check.setGravity(Gravity.CENTER);
check.setBackground(circleBackground(palette.accent, null));
...
tagRow.setPadding(0, dp(6), 0, 0);
addChip(tagRow, type.label, palette, true);
for (tagIndex = 0; tagIndex < rowTags.length && tagIndex < 3;
tagIndex += 1) {
addChip(tagRow, String(rowTags[tagIndex].name), palette, false);
}
if (rowTags.length > 3) {
addChip(tagRow, "+" + String(rowTags.length - 3), palette, false);
}
if (rowTags.length > 0) { state.renderedTagLabelCount += 1; }
tagScroll.addView(tagRow, new FrameLayout.LayoutParams(
FrameLayout.LayoutParams.WRAP_CONTENT,
FrameLayout.LayoutParams.WRAP_CONTENT));
center.addView(tagScroll, new LinearLayout.LayoutParams(
LinearLayout.LayoutParams.MATCH_PARENT,
LinearLayout.LayoutParams.WRAP_CONTENT));
...
right.addView(time, new LinearLayout.LayoutParams(dp(48), dp(18)));
star.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { selectRow(row); togglePinned(row); }
}));
right.addView(star, new LinearLayout.LayoutParams(dp(40), dp(34)));
if (allowReorder) {
more.setOnTouchListener(new JavaAdapter(View.OnTouchListener, {
onTouch: function (view, event) {
return handleReorderTouch(index, view, event);
}
}));
} else {
more.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { selectRow(row); openTagEditor(row); }
}));
}
right.addView(more, new LinearLayout.LayoutParams(dp(40), dp(32)));
card.addView(right, new LinearLayout.LayoutParams(dp(44),
LinearLayout.LayoutParams.MATCH_PARENT));
card.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { copyRow(row, true); }
}));
...
detailAction = makeText("详情", 1, palette.textPrimary, false);
detailAction.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { openDetail(row, false); }
}));
}
tagAction = makeText("标签", 1, palette.textPrimary, false);
tagAction.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { openTagEditor(row); }
}));
editAction = makeText("编辑", 1, palette.textPrimary, false);
editAction.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { openEditEditor(row); }
}));
deleteAction = makeText("删除", 1, palette.danger, false);
deleteAction.setOnClickListener(new JavaAdapter(View.OnClickListener, {
...
var logo = makeText("▣", 20, palette.accentStrong, true);
var title = makeText("全局剪切板", 18, palette.textPrimary, true);
...
var count = makeText("共 " + rows.length + " 条", 11,
palette.textSecondary, false);
var sort = makeText(active ? "筛选结果" : "↕  长按拖动可排序", 10,
palette.textTertiary, false);
var params;
...
logo.setBackground(roundedBackground(palette.accentSoft,
...
LinearLayout.LayoutParams.WRAP_CONTENT, 1));
headerPinView = makeIcon(ClipHub.Window.getState().pinned ? "⚑" : "⚐",
palette.icon, 19, "固定窗口");
headerPinView.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () {
ClipHub.Window.setPinned(!ClipHub.Window.getState().pinned);
renderRows(items);
}
}));
titleRow.addView(headerPinView,
new LinearLayout.LayoutParams(dp(40), dp(40)));
headerSettingsView = makeIcon("⚙", palette.icon, 18, "设置");
headerSettingsView.setOnClickListener(new JavaAdapter(View.OnClickListener, {
...
searchView = makeText("⌕   " + searchText, 12,
active ? palette.textPrimary : palette.textSecondary, false);
searchView.setGravity(Gravity.CENTER_VERTICAL);
searchView.setSingleLine(true);
searchView.setEllipsize(TextUtils.TruncateAt.END);
searchView.setPadding(dp(13), 0, dp(12), 0);
searchView.setBackground(roundedBackground(palette.surface,
active ? palette.accentBorder : palette.stroke, 22));
...
filterButton = makeText(active ? "≡  筛选中" : "≡  筛选", 11,
palette.accentStrong, true);
filterButton.setGravity(Gravity.CENTER);
filterButton.setBackground(roundedBackground(palette.accentSoft,
null, 14));
filterButton.setClickable(true);
filterButton.setFocusable(true);
filterButton.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { openFilterPanel(); }
}));
filterView = filterButton;
params = new LinearLayout.LayoutParams(dp(82), dp(44));
params.rightMargin = dp(8);
searchRow.addView(filterButton, params);
plusButton = makeIcon("+", palette.accentStrong, 25, "新增");
plusButton.setBackground(circleBackground(palette.accentSoft, null));
plusButton.setOnClickListener(new JavaAdapter(View.OnClickListener, {
...
statusRow.addView(count, new LinearLayout.LayoutParams(0,
LinearLayout.LayoutParams.WRAP_CONTENT, 1));
...
manageView = makeText("管理", 11, palette.accentStrong, true);
manageView.setGravity(Gravity.CENTER);
manageView.setClickable(true);
manageView.setFocusable(true);
manageView.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () {
var row = selectedRow();
if (row === null && items.length > 0) { row = items[0]; }
openTagEditor(row);
}
}));
statusRow.addView(manageView,
new LinearLayout.LayoutParams(dp(45), dp(32)));
container.addView(statusRow, new LinearLayout.LayoutParams(
LinearLayout.LayoutParams.MATCH_PARENT,
LinearLayout.LayoutParams.WRAP_CONTENT));
return container;
}
...
toolbar.setBackground(roundedBackground(palette.toolbar, null, 18));
bottomPinView = makeActionButton("⚑", "置顶", palette, false, false);
bottomPinView.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { var row = selectedRow(); if (row) {
togglePinned(row); } }
}));
bottomEditView = makeActionButton("✎", "编辑", palette, false, false);
bottomEditView.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { openEditEditor(selectedRow()); }
}));
bottomAddView = makeActionButton("+", "新增", palette, true, false);
bottomAddView.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { openNewEditor(); }
}));
bottomDeleteView = makeActionButton("⌫", "删除", palette, false, true);
bottomDeleteView.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { var row = selectedRow(); if (row) {
deleteRow(row); } }
}));
bottomTranslateView = makeActionButton("文", "翻译", palette, false, false);
bottomTranslateView.setOnClickListener(new JavaAdapter(View.OnClickListener, {
onClick: function () { var row = selectedRow(); if (row) {
openDetail(row, true); } }
}));
params = new LinearLayout.LayoutParams(0, dp(58), 1);
toolbar.addView(bottomPinView, params);
toolbar.addView(bottomEditView, new LinearLayout.LayoutParams(0, dp(58), 1));
toolbar.addView(bottomAddView, new LinearLayout.LayoutParams(0, dp(58), 1));
toolbar.addView(bottomDeleteView, new LinearLayout.LayoutParams(0, dp(58), 1));
toolbar.addView(bottomTranslateView, new LinearLayout.LayoutParams(0, dp(58), 1));
...
undoBar.setPadding(dp(10), dp(7), dp(8), dp(7));
undoBar.setBackground(roundedBackground(palette.surfaceMuted,
palette.stroke, 12));
undoText = makeText("已删除 1 条记录", 11,
palette.textSecondary, false);
undoBar.addView(undoText, new LinearLayout.LayoutParams(0,
LinearLayout.LayoutParams.WRAP_CONTENT, 1));
undoView = makePill("撤销", palette, true);
...
undoText.setPadding(dp(12), dp(42), dp(12), dp(42));
list.addView(undoText, new LinearLayout.LayoutParams(
LinearLayout.LayoutParams.MATCH_PARENT,
LinearLayout.LayoutParams.WRAP_CONTENT));
} else {
state.emptyVisible = false;
for (index = 0; index < rows.length; index += 1) {
rowTags = tagMap[String(rows[index].id)] || [];
params = new LinearLayout.LayoutParams(
LinearLayout.LayoutParams.MATCH_PARENT,
LinearLayout.LayoutParams.WRAP_CONTENT);
params.bottomMargin = dp(8);
list.addView(buildCard(rows[index], rowTags, index,
...
lastShowOptions.heightDp = Number(options.heightDp || 500);
...
visible = true;
refresh(false);
return { ok: true, visible: true, open: openResult,
state: getState() };
}
...
sourceIconCount: Number(state.sourceIconCount),
toolbarActionCount: Number(state.toolbarActionCount),
cardMoreButtonCount: Number(state.cardMoreButtonCount),
selectedItemId: selectedItemId,
selectedCount: selectedItemId === null ? 0 : 1,
...
headerPinButtonPresent: headerPinView !== null,
headerSettingsButtonPresent: headerSettingsView !== null,
...
state.pinThreadId = null;
state.pinThreadName = null;
state.addThreadId = null;
state.addThreadName = null;
state.editThreadId = null;
state.editThreadName = null;
state.tagThreadId = null;
state.tagThreadName = null;
state.filterThreadId = null;
state.filterThreadName = null;
state.detailActionThreadId = null;
state.detailActionThreadName = null;
state.detailAddThreadId = null;
state.detailAddThreadName = null;
state.detailRemoveThreadId = null;
state.detailRemoveThreadName = null;
state.reorderThreadId = null;
...
return row === null || row === undefined ? false :
openDetail(row, true);
},
