...
测试入口版本：5。锚点目标选择靠前的首个已脱水记录，避免定位测试本身进入页尾懒加载阈值。测试载荷使用带版本号的独立分片，旧入口分片不再被新版本覆盖；合并分片后先移除 Base64 空白字符，再执行 SHA-256 校验。数字分页测试页大小使用 180，确保保留窗口之外存在可脱水记录。
...
2026-08-07 真机返回 `ok = true`：初始 180 条只保留 72 条完整数据，108 条脱水；AJAX 追加到 362 条后只保留 34 条完整数据，328 条脱水。旧记录重新水合、标签恢复、缺失 ID 修正、锚点误差 0、回到最新、AJAX 与数字分页预加载均通过，Stage 8 记为 PASS。
