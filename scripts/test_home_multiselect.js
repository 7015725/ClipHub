...
c.headerMetrics=function(){return {};};c.homeNormalHeader=null;c.homeMultiHeader=null;c.homeMultiEntry=null;
c.circleBackground=function(){};c.ClipHub.Theme={applyTextColor:function(){}};
...
assert.strictEqual(c.homeMultiBar.visibility,8);
c.beginHomeMulti(); assert(c.homeMulti.active);
...
assert.strictEqual(holder.multiCheck.text,'✓'); holder.card.longClick();assert.strictEqual(tokenCount,1);
holder.card.click();assert.deepStrictEqual(Array.from(c.homeMultiIds()),[1],'consumed long press cannot toggle selection');
holder.row={id:2};holder.itemId=2;c.syncHomeMultiHolder(holder);
assert.strictEqual(holder.multiCheck.text,'');holder.multiCheck.click();assert.deepStrictEqual(Array.from(c.homeMultiIds()),[1,2]);
...
holder.card.click();holder.card.longClick();assert.strictEqual(inputCount,1);assert.strictEqual(tokenCount,2);
c.beginHomeMulti();
...
var actions=[];
c.fullResultRowById=function(row){return {id:row.id,content:'full text',is_sensitive:0};};
c.ClipHub.Clipboard={writeText:function(){actions.push('copy');return {ok:true};}};
c.ClipHub.Editor={openItem:function(){actions.push('edit');}};
c.ClipHub.Translation={openForItem:function(){actions.push('translate');}};
c.ClipHub.TokenizerUI={openFromHomeItem:function(){actions.push('tokenize');return true;}};
c.ClipHub.List.deleteItem=function(){actions.push('delete');return true;};
c.captureScrollAnchor=function(){};c.clearCopyFeedback=function(){};c.attachCopyFeedbackBanner=function(){};
assert.strictEqual(c.copyResultRow({id:1}),true);assert.strictEqual(c.editResultRow({id:1}),true);
assert.strictEqual(c.deleteResultRow({id:1}),true);assert.strictEqual(c.translateResultRow({id:1}),true);
assert.strictEqual(c.openTokenizerForResultRow({id:1}),true);
assert.deepStrictEqual(actions,['copy','edit','delete','translate','tokenize']);
[true,false].forEach(function(busy){c.homeMulti.busy=busy;c.homeMulti.confirmation=busy?null:{};
['copyResultRow','editResultRow','deleteResultRow','translateResultRow','openTokenizerForResultRow'].forEach(function(n){assert.strictEqual(c[n]({id:1}),false);});});
c.homeMulti.confirmation=null;assert.strictEqual(actions.length,5);assert.strictEqual(c.swipeInteractionBlocked(),true);
