...
Stage 9 v4 真机返回 `ok = true`：

- 新内容待处理期间 ID 与滚动位置完全不变；
- 回到最新后首条为新增记录、`scrollY = 0`、锚点为空；
- 删除、撤销、编辑、置顶和标签变化后锚点均为同一 ID，误差均为 `0px`；
- 标签、搜索和排序重建后的锚点误差均为 `0px`；
- 数字分页预期第 3 页、实际第 3 页，切回 AJAX 后锚点不变；
- 原始分页设置已恢复，所有模块错误字段均为空。

正式结论：Stage 9 PASS，可以进入 Stage 10 完整系统回归。
