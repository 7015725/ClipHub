/* ClipHub embedded Editor system Back probe 061. Rhino ES5 only.
 * Three-stage diagnostic:
 * 1) Run while Editor is open. Saves an in-editor baseline.
 * 2) Perform one system side-swipe and run again.
 *    If that swipe only hides IME, Probe 061 automatically re-baselines.
 * 3) Perform the next side-swipe and run again to capture the real failure.
 */
(function (global) {
    ...
    var PROBE_NAME = "cliphub_editor_embedded_back_probe_061";
    ...
    function snapshot(statusAck) {
        ...
        var diagnostics = status.runtimeDiagnostics || null;
        var removal = diagnostics && diagnostics.removal ? diagnostics.removal : {};
        return {
            capturedAt: now(),
    ...
    function saveBaseline(file, stage, snapshotValue) {
    ...
    function main() {
        ...
        var baselineFile = new File(cacheDir,
            "cliphub_editor_embedded_back_probe_061_baseline.json");
        ...
        if (String(manifest.moduleSetVersion || "") !== REQUIRED_SET) {
            throw new Error("Probe 061 requires module set " + REQUIRED_SET +
        ...
        if (!baselineFile.isFile()) {
            if (!current.editor.attached ||
                    current.editor.embeddedInPrimary !== true ||
                    !isEditorPage(current.shell.activePageId)) {
                return {
                    ok: true,
                    probe: PROBE_NAME,
                    probeVersion: 1,
                    phase: "waiting_for_editor_open",
                    instruction: "请先从首页进入编辑页并保持编辑页打开，然后再次运行 Probe 061",
                    current: current
                };
            }
            saveBaseline(baselineFile, "editor_open", current);
            return {
                ok: true,
                probe: PROBE_NAME,
                probeVersion: 1,
                phase: "baseline_saved_editor_open",
                instruction: "现在执行一次系统侧滑返回，然后保持当前界面不做其他操作，再次运行 Probe 061",
                keyboardVisible: current.editor.keyboardVisible,
                baselinePath: String(baselineFile.getAbsolutePath()),
                baseline: current
            };
        }

        baselineRecord = JSON.parse(read(baselineFile));
        before = baselineRecord.snapshot || {};
        change = delta(current, before);

        if (before.editor && before.editor.attached === true &&
                before.editor.keyboardVisible === true &&
                current.editor.attached === true &&
                current.editor.keyboardVisible === false &&
                change.navigation.backStartedCount === 0 &&
                change.navigation.backInvokedCount === 0 &&
                change.shellBackDispatchCount === 0) {
            saveBaseline(baselineFile, "post_ime_hide", current);
            return {
                ok: true,
                probe: PROBE_NAME,
                probeVersion: 1,
                phase: "ime_consumed_wait_second_swipe",
                classification: "FIRST_SWIPE_CONSUMED_BY_IME",
                instruction: "第一次侧滑仅收起了键盘。现在再执行一次系统侧滑返回，保持界面不做其他操作，然后第三次运行 Probe 061",
                delta: change,
                before: before,
                after: current
            };
        ...
        classification = classify(before, current, change);
        result = {
            ok: true,
            probe: PROBE_NAME,
            probeVersion: 1,
            phase: "comparison_complete",
            baselineStage: String(baselineRecord.stage || "unknown"),
    ...
    try {
        global.ClipHubEditorEmbeddedBackProbe061Result = main();
    } catch (error) {
        global.ClipHubEditorEmbeddedBackProbe061Result = {
            ok: false,
            probe: PROBE_NAME,
            probeVersion: 1,
            error: String(error)
...
JSON.stringify(ClipHubEditorEmbeddedBackProbe061Result);
