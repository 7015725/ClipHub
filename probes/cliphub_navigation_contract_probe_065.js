...
(function (global) {
    ...
    function finish(runtimeDir, probeDir, result) {
        ...
        toast(result.copiedToClipboard ?
            "Probe 065 完成，结果已复制到剪贴板" :
            "Probe 065 完成，请读取 probes 输出文件");
        return result;
    }
    ...
    function monitor() {
        ...
        if (String(manifest.moduleSetVersion || "") !== REQUIRED_SET) {
            throw new Error("Probe 065 requires module set " + REQUIRED_SET +
                ", current=" + String(manifest.moduleSetVersion || ""));
        }
        if (String(manifest.sourceRef || "") !== REQUIRED_REF) {
            throw new Error("Probe 065 requires sourceRef " + REQUIRED_REF +
                ", current=" + String(manifest.sourceRef || ""));
        }

        toast("Probe 065 已启动，请在 45 秒内打开 ClipHub 编辑页");
