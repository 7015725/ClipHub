/* ClipHub new home UI probe 033 implementation. Rhino ES5 only. */
(function (global) {
    ...
    function add(content, contentType, sourcePackage, sourceLabel,
        ...
        return Number(global.ClipHub.Repository.insertItem({
            ...
            sourcePackage: String(sourcePackage || "com.cliphub.probe033"),
    ...
    function main() {
        ...
        var outputFile = new File(ensureDir(new File(formal, "probes")),
            "cliphub_new_home_ui_probe_033_" + stamp(startedAt) + ".json");
        ...
        var result = {
            ok: false,
            probe: "cliphub_new_home_ui_probe_033",
            probeVersion: 1,
            ...
            instruction: "显示新首页后请截取一张未裁剪原始截图，等待探测自动结束。",
        ...
        try {
            ...
            ids.push(add("https://developer.android.com/", "url",
                "com.android.chrome", "Chrome 浏览器", false, false,
                baseTime + 1000));
            ids.push(add("会议在明天下午三点，地点是B栋会议室，请准时参加。",
                "text", "com.tencent.mm", "微信", false, false,
                baseTime + 2000));
            pinnedId = add("+86 138 0013 8000", "phone",
                "com.android.contacts", "联系人", false, true,
                baseTime + 3000);
            ids.push(pinnedId);
            ids.push(add("design.team@example.com", "email",
                "com.google.android.gm", "Gmail", false, false,
                baseTime + 4000));
            ids.push(add("var greet = function (name) { return 'Hello, ' + name; };",
                "text", "com.termux", "Termux", false, false,
                baseTime + 5000));
            ids.push(add("Android 14 引入了对预测性返回动画的支持，提升了用户体验和应用流畅度。",
                "text", "com.google.android.apps.docs", "文档", false,
                false, baseTime + 6000));
            tagWork = Number(global.ClipHub.Repository.ensureTag("工作"));
            ...
            result.seededCount = Number(global.ClipHub.Repository.countItems(false));
            result.tagCount = Number(global.ClipHub.Repository.listTags().length);
            ...
            result.homeReady = waitFor(function () {
                ...
                return global.ClipHub.Window.getState().attachedToWindow === true &&
                    ...
                    state.homeStyle === "reference_home_v1" &&
                    state.toolbarActionCount === 5;
            }, 1800);
            result.selectionClick = global.ClipHub.List.performItemClick(2);
            waitFor(function () {
                return global.ClipHub.List.getState().selectedCount === 1;
            }, 1000);
            windowState = global.ClipHub.Window.getState();
            listState = global.ClipHub.List.getState();
            navigationState = global.ClipHub.Navigation.getState();
            result.windowState = windowState;
            result.listState = listState;
            result.navigationState = navigationState;
            result.windowInsideSafeBounds = insideSafeBounds(windowState);
            ...
            result.ok = result.error === null && result.start &&
                ...
                Number(result.windowState.dimAmount) >= 0.35 &&
    ...
    try {
        global.ClipHubNewHomeUiProbe033Result = main();
    } catch (error) {
        global.ClipHubNewHomeUiProbe033Result = {
            ok: false,
            probe: "cliphub_new_home_ui_probe_033",
            probeVersion: 1,
            fatal: true,
...
JSON.stringify(ClipHubNewHomeUiProbe033Result);
