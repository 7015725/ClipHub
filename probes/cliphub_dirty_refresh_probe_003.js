/* ClipHub hidden-dirty refresh and rapid show/hide probe 003. Rhino ES5 only. */
(function (global) {
    ...
    var PROBE = "cliphub_dirty_refresh_probe_003";
    var PROBE_VERSION = 3;
    ...
    function main() {
        ...
        var initialClipboardText = readClipboardText(clipboardManager);
        ...
        try {
            ...
            sendCommand(runtimeDir, endpoint, "hide", "prepare-hide");
            toast("ClipHub 阶段2：请在60秒内复制一段新的文本");
            ...
            result.ok = result.finalShow.windowCacheReused === true &&
    ...
    try {
        global.ClipHubDirtyRefreshProbe003Result = main();
    } catch (error) {
        global.ClipHubDirtyRefreshProbe003Result = {
            ok: false,
            probe: PROBE,
            probeVersion: PROBE_VERSION,
...
JSON.stringify(ClipHubDirtyRefreshProbe003Result);
